...
# Fixed practical proportions for small rooms (width, depth) - never stretched
FIXED_PROPORTIONS = {
    "attached bath":  (5.0, 7.0),
    "dressing":       (5.0, 6.0),
    "laundry":        (5.0, 6.0),
    "store":          (5.0, 6.0),
    "foyer":          (6.0, 8.0),
...
def draw_bubble_diagram(G, variant_label, out_path, info_lines=None):
    ...
    nx.draw_networkx_labels(G, pos, ax=ax, font_size=FONT_SIZES["bubble_label"], font_weight="bold")
    ax.set_title(f"Room Relationship Diagram - {variant_label}", fontsize=FONT_SIZES["title"], fontweight="bold")
    ax.axis("off")
    ...
    if handles:
        ax.legend(handles=handles, loc="upper left", bbox_to_anchor=(1.01, 1.0),
                   fontsize=FONT_SIZES["legend"], frameon=False, title="Room type")

    if info_lines:
        text = "\n".join(info_lines)
        ax.text(1.01, 0.45, text, transform=ax.transAxes, fontsize=FONT_SIZES["info_text"],
...
def _draw_room_piece(ax, piece, face, edge, alpha, hatch, label, area_label, irregular=False):
    ...
    if irregular:
        text += "\n(verify on site)"
    ax.text(cx, cy, text, ha="center", va="center", fontsize=FONT_SIZES["room_label"], fontweight="bold",
...
def _draw_exhaust_marker(ax, piece, buildable_poly, label):
    ...
    if dist < 0.75:
        ...
        ax.text(sx + ux * length * 1.3, sy + uy * length * 1.3, "EXH", color="#d62828",
                fontsize=FONT_SIZES["exhaust_label"], fontweight="bold", ha="center", va="center")
        return True
    else:
        ax.plot([cx, b_near.x], [cy, b_near.y], linestyle="--", color="#888888", linewidth=1.1)
        ax.text(b_near.x, b_near.y, "duct", color="#888888", fontsize=FONT_SIZES["duct_label"], style="italic",
                ha="center", va="center")
        return False
...
def _constrain_suite_rects(suite_block, rx, ry, rw, rh):
    """
    Subdivide a suite rectangle into bedroom + bath + dressing while
    preserving sensible proportions. Bedrooms avoid long/narrow shapes.
    Small rooms (bath, dressing) keep fixed practical proportions.
    """
    members = suite_block["members"]
    bed = next((m for m in members if m["type"] == "bedroom"), None)
    bath = next((m for m in members if m["type"] == "attached bath"), None)
    dress = next((m for m in members if m["type"] == "dressing"), None)

    if not bed or not bath or not dress:
        sub_vals = [m["area"] for m in members]
        return squarify(sub_vals, rx, ry, rw, rh)

    bath_w, bath_d = FIXED_PROPORTIONS["attached bath"]
    dress_w, dress_d = FIXED_PROPORTIONS["dressing"]

    if rw >= rh:
        side_width = max(bath_w, dress_w)
        side_width = min(side_width, rw * 0.35)
        side_width = max(side_width, min(bath_w, dress_w))

        bed_w = rw - side_width
        bed_d = rh

        bed_w = max(bed_w, BEDROOM_CONSTRAINTS["min_w"])
        bed_d = max(bed_d, BEDROOM_CONSTRAINTS["min_d"])
        ratio = max(bed_w / bed_d, bed_d / bed_w) if min(bed_w, bed_d) > 0 else 1
        if ratio > BEDROOM_CONSTRAINTS["max_ratio"]:
            if bed_w > bed_d:
                bed_w = min(bed_w, bed_d * BEDROOM_CONSTRAINTS["max_ratio"])
            else:
                bed_d = min(bed_d, bed_w * BEDROOM_CONSTRAINTS["max_ratio"])

        side_width = rw - bed_w
        side_width = max(side_width, min(bath_w, dress_w))
        bed_w = rw - side_width

        bath_h = rh * (bath["area"] / (bath["area"] + dress["area"])) if (bath["area"] + dress["area"]) > 0 else rh / 2
        dress_h = rh - bath_h
        bath_h = max(bath_h, bath_d)
        dress_h = max(dress_h, dress_d)
        if bath_h + dress_h > rh:
            scale = rh / (bath_h + dress_h)
            bath_h *= scale
            dress_h *= scale
        ...
        for m in members:
            if m["type"] == "bedroom":
                result.append((rx, ry, bed_w, rh))
            elif m["type"] == "attached bath":
                result.append((rx + bed_w, ry, side_width, bath_h))
            elif m["type"] == "dressing":
                result.append((rx + bed_w, ry + bath_h, side_width, dress_h))
        return result
    else:
        side_depth = max(bath_d, dress_d)
        side_depth = min(side_depth, rh * 0.35)
        side_depth = max(side_depth, min(bath_d, dress_d))
        ...
        bath_w = rw * (bath["area"] / (bath["area"] + dress["area"])) if (bath["area"] + dress["area"]) > 0 else rw / 2
        dress_w = rw - bath_w
        bath_w = max(bath_w, bath_w)
        dress_w = max(dress_w, dress_w)
        if bath_w + dress_w > rw:
            scale = rw / (bath_w + dress_w)
            bath_w *= scale
            dress_w *= scale
        ...
        for m in members:
            if m["type"] == "bedroom":
                result.append((rx, ry + side_depth, bed_w, bed_d))
            elif m["type"] == "attached bath":
                result.append((rx, ry, bath_w, side_depth))
            elif m["type"] == "dressing":
                result.append((rx + bath_w, ry, dress_w, side_depth))
        return result


def _draw_circulation_graph(ax, corridor_pieces, room_pieces, entrance_side, buildable_poly):
    """
    Draw a subtle circulation connectivity graph from entrance through
    foyer/corridors to every room. Bedrooms link to attached baths.
    Paths can be straight, L-, T-, plus-, or branched.
    """
    if not corridor_pieces:
        return
    ...
    entrance_pt = Point(entrance_points.get(entrance_side, ((minx + maxx) / 2, miny)))

    nearest_corr = None
    min_dist = float('inf')
    for piece in corridor_pieces:
        np_piece, np_corr = nearest_points(entrance_pt, piece)
        d = np_piece.distance(np_corr)
        if d < min_dist:
            min_dist = d
            nearest_corr = np_corr

    if nearest_corr is None:
        return

    ax.annotate("", xy=(nearest_corr.x, nearest_corr.y),
                xytext=(entrance_pt.x, entrance_pt.y),
                arrowprops=dict(arrowstyle="->", color="#2a9d8f", lw=1.5,
                               connectionstyle="arc3,rad=0.1"))
    ax.text(entrance_pt.x, entrance_pt.y, "ENTRANCE", color="#2a9d8f",
            fontsize=FONT_SIZES["annotation"], fontweight="bold",
            ha="center", va="center",
            bbox=dict(boxstyle="round,pad=0.2", facecolor="white", alpha=0.8, edgecolor="#2a9d8f"))

    room_centroids = {}
    for label, is_bed, piece in room_pieces:
        cx, cy = piece.centroid.x, piece.centroid.y
        room_centroids[label] = (cx, cy)

    room_to_corr = {}
    for label, (cx, cy) in room_centroids.items():
        best_pt = None
        best_dist = float('inf')
        for piece in corridor_pieces:
            np_room, np_corr = nearest_points(Point(cx, cy), piece)
            d = np_room.distance(np_corr)
            if d < best_dist:
                best_dist = d
                best_pt = np_corr
        if best_pt:
            room_to_corr[label] = (best_pt, best_dist)

    drawn_pairs = set()
    for label, (corr_pt, dist) in room_to_corr.items():
        cx, cy = room_centroids[label]
        dx = abs(cx - corr_pt.x)
        dy = abs(cy - corr_pt.y)

        if max(dx, dy) > min(dx, dy) * 2.5:
            if dx > dy:
                mid_x = (corr_pt.x + cx) / 2
                path_x = [corr_pt.x, mid_x, mid_x, cx]
                path_y = [corr_pt.y, corr_pt.y, cy, cy]
            else:
                mid_y = (corr_pt.y + cy) / 2
                path_x = [corr_pt.x, corr_pt.x, cx, cx]
                path_y = [corr_pt.y, mid_y, mid_y, cy]
            ax.plot(path_x, path_y, color="#264653", linewidth=0.8, alpha=0.4, linestyle="-")
        else:
            ax.plot([corr_pt.x, cx], [corr_pt.y, cy], color="#264653", linewidth=0.8, alpha=0.4, linestyle="-")

    for label, is_bed, piece in room_pieces:
        if is_bed:
            bath_label = label.replace("Bedroom", "Attached Bath")
            if bath_label in room_centroids:
                bx, by = room_centroids[bath_label]
                cx, cy = room_centroids[label]
                ax.plot([cx, bx], [cy, by], color="#e76f51", linewidth=1.0, alpha=0.5, linestyle="--")
...
def draw_floor_plan(variant, buildable_poly, plot_poly, entrance_side, style_idx, out_path):
            ...
            for piece, _irregular in pieces:
                ...
                if piece.area > 12:
                    ...
                    ax.text(cx, cy, "Corridor /\nCirculation", ha="center", va="center",
                            fontsize=FONT_SIZES["corridor_label"], fontweight="bold",
                            bbox=dict(boxstyle="round,pad=0.25", facecolor="white", alpha=0.75, linewidth=0))
            continue

        if block["kind"] == "suite" and len(block["members"]) > 1:
            sub_rects = _constrain_suite_rects(block, rx, ry, rw, rh)
    ...
    _draw_circulation_graph(ax, corridor_pieces, room_pieces, entrance_side, buildable_poly)
    ...
    ax.set_xlabel("Feet", fontsize=FONT_SIZES["annotation"])
    ax.set_ylabel("Feet", fontsize=FONT_SIZES["annotation"])
    ...
    ax.legend(handles=handles, loc="upper left", bbox_to_anchor=(1.01, 1.0),
              fontsize=FONT_SIZES["legend"], frameon=False, title="Legend")
    ...
    ax.text(1.01, 0.40, info_text, transform=ax.transAxes, fontsize=FONT_SIZES["info_text"],
            va="top", ha="left", family="monospace",
            bbox=dict(boxstyle="round,pad=0.5", facecolor="#f7f7f7", edgecolor="#cccccc"))
