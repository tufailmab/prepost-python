...
dxf_plot_advisor.py
--------------------
Built to run straight from IDLE (or any editor) - just edit the CONFIG
block below, then press Run (F5).

Give it ANY boundary DXF (a plot/site outline made of LINE or LWPOLYLINE
entities) and it reports:

  1. Plot area, perimeter, bounding dimensions, and shape regularity
  2. Buildable area after a chosen setback
  3. MULTIPLE suggested room-program variants (e.g. Compact / Balanced /
     Spacious) that fit the buildable area, using standard residential
     room-size guidelines (editable in ROOM_STANDARDS below)
  4. A room-relationship "bubble diagram" (PNG) for each variant, showing
     which rooms should sit next to which - accept the built-in
     architectural defaults, or customize connections in ROOM_CONNECTIONS
  5. An actual space-filling FLOOR PLAN (PNG) for each variant, now with:
       - a USER-CHOSEN entrance side (N/S/E/W or auto)
       - wet rooms (bath / kitchen / laundry / etc.) deliberately packed
         against the buildable boundary so they get a real exterior wall,
         with a small exhaust-vent marker drawn on that wall
       - an explicit corridor strip (real width, not just a % fudge)
         connecting the entrance to the rest of the house
       - a "sensible shape" cleanup pass that catches slivers/odd offcuts
         produced when clipping to an irregular plot and folds them back
         into circulation instead of leaving a weird little room fragment
  6. If the DXF already contains labelled room rectangles on a layer you
     name (e.g. "ROOMS"), it audits each room against minimum size
     standards and flags anything undersized or oddly proportioned

If DXF_PATH is left as None, the script looks for a .dxf file in the
current working directory automatically.

Requires: ezdxf, shapely, networkx, matplotlib
    pip install ezdxf shapely networkx matplotlib --break-system-packages
"""
import glob
import math
import sys
...
def _style_order(rooms_tier_groups, style_idx):
    """`rooms_tier_groups` = (foyer_blocks, suite_blocks, public_blocks).
    Returns one list, ordering the groups differently per style so each
    plan variant genuinely rearranges the house rather than just scaling
    the same layout:
      style 0 - public rooms (kitchen/lounge/guest/study/laundry) right
                after the foyer, bedroom wing tucked behind them
      style 1 - each group sorted by area but public/private interleaved
                largest-first, a more even mix around the entrance
      style 2 - bedroom wing pulled up right after the foyer, public
                rooms tucked behind - mirror of style 0
    """
    foyer_blocks, suite_blocks, public_blocks = rooms_tier_groups
    suite_sorted = sorted(suite_blocks, key=lambda b: -b["area"])
    public_sorted = sorted(public_blocks, key=lambda b: -b["area"])
    if style_idx % 3 == 0:
        return foyer_blocks + public_sorted + suite_sorted
    elif style_idx % 3 == 1:
        merged = sorted(suite_blocks + public_blocks, key=lambda b: -b["area"])
        return foyer_blocks + merged
    else:
        return foyer_blocks + suite_sorted + public_sorted


def draw_floor_plan(variant, buildable_poly, plot_poly, entrance_side, style_idx, out_path):
    """Single compact squarified pack of the WHOLE buildable footprint (no
    forced perimeter band, so no dead middle void). Entrance/exterior-need
    rooms are simply placed FIRST in the fill order, which - because each
    squarify row spans the full remaining width or height of the region -
    lands them against a real edge of the plot; which edge is controlled
    by mirroring the pack into the bbox corner that belongs to
    entrance_side (see CORNER_FLIP). Interior-tolerant rooms (store,
    stair) and the corridor/circulation leftover go last, so they're the
    ones most likely to end up deep in the plan - which is exactly where
    you want them. `style_idx` reorders which room group (public vs
    bedroom wing) comes right after the foyer, so each plan variant gets
    a genuinely different arrangement rather than the same layout scaled.
    """
    minx, miny, maxx, maxy = buildable_poly.bounds
    bbox_w, bbox_h = maxx - minx, maxy - miny
    if bbox_w <= 0 or bbox_h <= 0:
        raise ValueError("Buildable area has zero extent - setback may be too large.")
    ...
    interior_blocks = sorted([b for b in blocks if not b["exterior"]], key=lambda b: -b["area"])
    ...
    # keep a genuine minimum corridor: CORRIDOR_WIDTH_FT x a rough run
    # length across the plot, so the % isn't just an arbitrary fudge
    min_corridor = min(CORRIDOR_WIDTH_FT * (bbox_w + bbox_h) * 0.5, target_total * 0.22)
    min_corridor = max(min_corridor, target_total * 0.04)

    if room_area_sum > target_total - min_corridor:
        shrink = (target_total - min_corridor) / room_area_sum if room_area_sum > 0 else 1
        for b in room_and_interior:
            b["area"] *= shrink
        room_area_sum = target_total - min_corridor

    circulation_area = max(target_total - room_area_sum, target_total * 0.02)
    circulation_block = {"label": "Corridor / Circulation", "area": circulation_area,
                          "members": None, "kind": "circulation"}

    all_items = rooms_ordered + [circulation_block] + interior_blocks
    values = [b["area"] for b in all_items]

    # pack in a local (0,0)-origin frame, then mirror into the bbox
    # corner that matches entrance_side
    local_rects = squarify(values, 0, 0, bbox_w, bbox_h, fill_order=list(range(len(values))))
    flip_x, flip_y = CORNER_FLIP[entrance_side]
    final_rects = []
    for (lx, ly, lw, lh) in local_rects:
        rx = minx + ((bbox_w - lx - lw) if flip_x else lx)
        ry = miny + ((bbox_h - ly - lh) if flip_y else ly)
    ...
    for block, rect in zip(all_items, final_rects):
        ...
        if block["kind"] == "circulation":
            ...
            for piece, _irregular in pieces:
                xs, ys = piece.exterior.xy
                ax.fill(xs, ys, facecolor="#e8e8e8", edgecolor="#aaaaaa", linewidth=1.2,
                        alpha=0.6, hatch="///")
                if piece.area > 12:
                    cx, cy = piece.centroid.x, piece.centroid.y
                    ax.text(cx, cy, "Corridor /\nCirculation", ha="center", va="center",
        ...
        if block["kind"] == "suite" and len(block["members"]) > 1:
            ...
            for member, srect in zip(block["members"], sub_rects):
                ...
                for piece, irregular in pieces:
                    ...
                    _draw_room_piece(ax, piece, TYPE_COLORS.get(member["type"], "#cccccc"),
                                      "#333333", 0.85, None, member["name"], member["area"], irregular)
        ...
        for piece, irregular in pieces:
            irregular_count += 1 if irregular else 0
            _draw_room_piece(ax, piece, face, "#333333", 0.85, None,
                              member["name"], member["area"], irregular)
            if member["type"] in VENT_ROOM_TYPES:
                if not _draw_exhaust_marker(ax, piece, buildable_poly, member["name"]):
                    unvented.append(member["name"])
        legend_types.add(member["type"])
    ...
    if circulation_extra > 8:
        print(f"  Note: ~{circulation_extra:,.0f} sqft of odd offcuts against the "
              f"irregular boundary were folded into open/circulation space rather "
              f"than drawn as undersized rooms.")
    if unvented:
        print(f"  Caution [{variant['label']}]: {', '.join(sorted(set(unvented)))} "
              f"didn't land on an exterior wall in this arrangement - shown with a "
              f"grey 'duct' line instead of a wall vent. Try a different "
              f"ENTRANCE_SIDE, or increase CORRIDOR_WIDTH_FT/shrink room count, and re-run.")

    ax.set_aspect("equal")
    pad_x, pad_y = bbox_w * 0.08, bbox_h * 0.08
    if plot_poly is not None:
    ...
    ax.set_title(f"Floor Plan - {variant['label']}\n"
                 f"({variant['n_bed']} Bedroom suite(s), {len(variant['instances'])} rooms, "
    ...
    handles = [plt.Rectangle((0, 0), 1, 1, facecolor=TYPE_COLORS.get(t, "#cccccc"),
                              edgecolor="#333333", alpha=0.85, label=t.title())
               for t in sorted(legend_types)]
    handles.append(plt.Rectangle((0, 0), 1, 1, facecolor="#e8e8e8", edgecolor="#aaaaaa",
                                  hatch="///", label="Corridor / Circulation"))
    ...
    info_text = (
        ...
        f"Corridor width target: {CORRIDOR_WIDTH_FT:.1f} ft\n"
        f"Layout style: {style_idx % 3 + 1} of 3\n"
        f"Fit: {'OK' if variant['fits'] else 'TIGHT'}"
    )
    ax.text(1.01, 0.40, info_text, transform=ax.transAxes, fontsize=8.5,
            va="top", ha="left", family="monospace",
            bbox=dict(boxstyle="round,pad=0.5", facecolor="#f7f7f7", edgecolor="#cccccc"))
...
def main():
    ...
    for i in range(n_plans):
        label, circ, bed_delta = PLAN_PRESETS[i % len(PLAN_PRESETS)]
        extra_cycles = i // len(PLAN_PRESETS)
        base_bed = args.bedrooms if args.bedrooms else None
        if base_bed:
            bed_wanted = base_bed + bed_delta + extra_cycles
        else:
            bed_wanted = None
        variant = build_program_variant(buildable.area, bed_wanted, circ,
                                         f"{label}" + (f" #{extra_cycles+1}" if extra_cycles else ""))
        if not base_bed and bed_delta:
            variant = build_program_variant(buildable.area, variant["n_bed"] + bed_delta, circ, variant["label"])
        variant["style_idx"] = i
